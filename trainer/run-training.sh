...
python trainer/preprocess.py \
   ...
   --input_path "${GCS_PATH}/parts_images_validate.csv" \
   --output_path "gs://${PROJECT}-training/${MODEL_NAME}/${MODEL_VERSION}/preproc/eval" \
   --job_name $(echo "${MODEL_VERSION}-eval" | sed 's/_/-/g') \
   --num_workers 1 \
   --max_num_workers 1 \
   ...
   --profile_location "${PROJECT}-training/${MODEL_NAME}/${MODEL_VERSION}/preproc-profile" \
   --cloud &

python trainer/preprocess.py \
   --input_dict "$DICT_FILE" \
   --input_path "${GCS_PATH}/parts_images_train.csv" \
   --output_path "gs://${PROJECT}-training/${MODEL_NAME}/${MODEL_VERSION}/preproc/train" \
   --job_name $(echo "${MODEL_VERSION}-train" | sed 's/_/-/g') \
   --num_workers 1 \
   --max_num_workers 1 \
   --autoscaling_algorithm "THROUGHPUT_BASED" \
   --zone "us-central1-a" \
...
# wait for second preprocess job to complete - as it is longer
wait
...
gcloud ml-engine jobs submit training "$MODEL_VERSION" \
  --stream-logs \
  ...
  --eval_data_paths "gs://${PROJECT}-training/${MODEL_NAME}/${MODEL_VERSION}/preproc/eval*" \
  --train_data_paths "gs://${PROJECT}-training/${MODEL_NAME}/${MODEL_VERSION}/preproc/train*" \
  --label_count=3
...
# gcloud ml-engine versions create "$MODEL_VERSION" \
  # --model "$MODEL_NAME" \
  # --origin "${GCS_PATH}/training-files/${JOB_ID_DASHES}/training/model" \
  # --runtime-version=1.0
...
# gsutil cp \
  # ${BUCKET}/part01/part01.0005.jpg \
  # test.jpg

# Since the image is passed via JSON, we have to encode the JPEG string first.
# python -c 'import base64, sys, json; img = base64.b64encode(open(sys.argv[1], "rb").read()); print json.dumps({"key":"0", "image_bytes": {"b64": img}})' hfe.jpg &> request.json
